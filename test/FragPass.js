...
contract('FragPass', (accounts) => {
    ...
    beforeEach(async () => {
        contractInstance = await FragPass.new(root);
    });
    it('should mint a FragPass', async () => {
        const leaf = keccak256(alice);
        const proof = tree.getHexProof(leaf);

        const result = await contractInstance.safeMint(proof, {from: alice});
        expect(result.receipt.status).to.equal(true);

        expect(result.logs[0].event).to.equal('Transfer');
        expect(result.logs[0].args).to.have.property('from', '0x0000000000000000000000000000000000000000');
        expect(result.logs[0].args).to.have.property('to', alice);
        expect(result.logs[0].args).to.have.property('tokenId');
        expect(result.logs[0].args.tokenId.toNumber()).to.equal(0);

        expect(result.logs[1].event).to.equal('AccountClaimed');
        expect(result.logs[1].args).to.have.property('account', alice);
    })
    it('should not mint a FragPass if not part of whitelist', async () => {
        const leaf = keccak256(alice);
        const proof = tree.getHexProof(leaf);

        await utils.shouldThrow(contractInstance.safeMint(proof, {from: bob}));
    })
    it('should not mint a FragPass if already minted once', async () => {
        const leaf = keccak256(alice);
        const proof = tree.getHexProof(leaf);

        await contractInstance.safeMint(proof, {from: alice});
        await utils.shouldThrow(contractInstance.safeMint(proof, {from: alice}));
    })
})
